...
# Анимация (аналогично предыдущему коду)
...
with writer.saving(fig, "rk4_three_body.gif", 100):
    for i in range(n_steps):
        ax.clear()
        ax.set_xlim(-5, 5)
        ax.set_ylim(-5, 5)
...
plt.title('Энергии системы (метод Рунге-Кутты 4-го порядка)')
plt.xlabel('Время')
plt.ylabel('Энергия')
plt.legend()
plt.grid(True)
plt.savefig('rk4_energies.png')
plt.show()